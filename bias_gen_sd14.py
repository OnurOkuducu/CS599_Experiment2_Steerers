...
def activation_modulation_across_prompt(blocks_to_save, steer_prompt, strength, steps, guidance_scale, seed):
    print('Steer Prompt: ', steer_prompt)
...
sae = SparseAutoencoder.load_from_disk(os.path.join("Checkpoints/text_encoder.text_model.encoder.layers.9_k32_hidden3072_auxk256_bs4096_lr0.0004_2025-11-29T20:27:20.854708", '60000')).to('cuda', dtype=dtype) #exp4, layer 9 
...
i = args.start_iter
n_samples = len(data) * NUM_IMAGES_PER_PROMPT
...
while i < n_samples and i< args.end_iter:
    ...
    for a in range(NUM_IMAGES_PER_PROMPT):
        seed += a
        with torch.no_grad():
            ...
            for j in range(batch_size):
                ...
                image.save(f"{outdir}/{str(prompt)+'_'+str(a)}.png")            
    i += batch_size 
